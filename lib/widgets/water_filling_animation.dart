...
class WaveCircularLoader extends StatelessWidget {
  const WaveCircularLoader({Key? key}) : super(key: key);
      ...
      child: AutomatedAnimator(
        ...
        buildWidget: (double animationPosition) {
          return WaveLoadingBubble(
            foregroundWaveColor: const Color(0xFF6AA0E1),
            backgroundWaveColor: const Color(0xFF4D90DF),
            loadingWheelColor: const Color(0xFF77AAEE),
...
class AutomatedAnimator extends StatefulWidget {
  ...
  @override
  State<AutomatedAnimator> createState() => _AutomatedAnimatorState();
}
...
class _AutomatedAnimatorState extends State<AutomatedAnimator>
  ...
  void initState() {
    ...
    if (widget.doRepeatAnimation == true) controller.repeat(reverse: true);
  }
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (__, _) => widget.buildWidget(controller.value),
    );
  }
}
...
//* sub animations with different periodicities that are able to
...
//!disable for web or to remove outer moving path
    Path insetCirclePath = Path()
      ..addRRect(RRect.fromLTRBXY(
          -insetBubbleRadius,
          -insetBubbleRadius,
          insetBubbleRadius,
    ...
    canvas.drawPath(
        Path.combine(
            PathOperation.difference, loadingCirclePath, insetCirclePath),
        loadingCirclePaint);
    //! remove till here
